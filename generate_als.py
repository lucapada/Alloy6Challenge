...
xls_path = questionary.path("Please provide the (relative) path of the Excel file generated by Microsoft Forms (use ./../../):").ask()
...
personcode_is_email = questionary.confirm("Is the Person Code part of an email address (e.g. PERSONCODE@polimi.it)?").ask()
# 2.4 chiedo all'utente quali sono le colonne che contengono le risposte
answer_columns = questionary.checkbox("Please select all the columns containing the Alloy code to be merged in a unique .als file:", choices=columns).ask()
...
if not id_column or not personcode_column or not answer_columns:
    ...
    working_folder_name = questionary.text("Please provide a name for the folder where all the files will be saved:").ask()
    ...
    if not working_path or not working_folder_name:
        ...
        group_by_teacher = questionary.confirm("Do you want to group the files by teacher?").ask()
